...
FORMAT_MESSAGE = "\nTo log expense - enter message in next format to log expense: {amount};{currency_3_chars};{category};{place}, example: 15;USD;Food;McDonalds.".freeze
...
class ReportBuilder
  def self.build_from(spendy_reporting_response)
    begin
      spends = JSON.parse(spendy_reporting_response.body)['spends']
      if spends.empty?
...
Telegram::Bot::Client.run(Settings.app[:telegram_token]) do |bot|
  ...
  bot.listen do |message|
    if message.text == '/start'
      puts '/start'
      menu = "#{GREETINGS.sample}, #{message.from.first_name}!\n"
      menu << "/category_report - spend report by category in text.\n"
      menu << "/daily_report - spend report by day for current month in text.\n"
      menu << FORMAT_MESSAGE
      bot.api.send_message(chat_id: message.chat.id, text: menu)
    elsif message.text == '/category_report'
      response = RestClient.get("#{Settings.app[:spendy_reporting_url]}spend_by_category?user_name=#{message.from.username}")
      response_message = ReportBuilder.build_from(response)
      puts response_message
      bot.api.send_message(chat_id: message.chat.id, text: response_message)
    elsif message.text == '/daily_report'
      response = RestClient.get("#{Settings.app[:spendy_reporting_url]}spend_by_day?user_name=#{message.from.username}")
      response_message = ReportBuilder.build_from(response)
      puts response_message
      bot.api.send_message(chat_id: message.chat.id, text: response_message)
